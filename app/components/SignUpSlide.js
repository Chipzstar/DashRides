...
const signUpSlide = ({ item, isPassword, onChangeHandler, onNext, onBack, onSubmit, errors }) => (
	<Block style={[{ width, height }, styles.signUpContainer]}>
		<Block>
			<Text color={Theme.COLOURS.WHITE} style={styles.title}>
				{item.title}
			</Text>
			<Input
			...
			/>
			{errors[item.key] && (
				<Text bold size={18} color={Theme.COLOURS.BLACK}>
					{errors[item.key]}
				</Text>
			)}
			{isPassword && (
			...
			{isPassword && errors['confirmPassword'] && (
				<Text bold size={18} color={Theme.COLOURS.BLACK}>
		...
		<Block style={[{ alignSelf: 'flex-end', textAlign: 'right' }, !onBack && { padding: 20 }]}>
			{onBack && (
				<TouchableOpacity onPress={onBack}>
					<Text style={{ padding: 20 }} color={Theme.COLOURS.WHITE}>
						Go back
					</Text>
				</TouchableOpacity>
			)}
